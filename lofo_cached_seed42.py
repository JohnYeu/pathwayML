#!/usr/bin/env python3
"""Fast LOFO diagnostic using cached seed-42 samples and ALL filtered GO terms.

Uses all frequency-filtered GO terms (not MI-selected) to avoid supervised
feature-selection leakage across family boundaries. Lightweight XGBoost
(30 trees, hist method) for speed.
"""
...
def main():
    data=core.load_data()
    # Load saved seed-42 samples to avoid slow regeneration
    records=json.load(open(TABLE_DIR/'reproducibility'/'samples.json'))
    pos=[]; neg=[]; fam_rows=[]
    for rec in records:
        if rec['label']==1:
    ...
    # Use ALL frequency-filtered GO terms (not MI-selected) to avoid label-based leakage
    selected=list(data.go_terms)
    all_records=pos+neg
    t0=time.time(); X,names,groups=core.build_feature_matrix(all_records,selected,data,seed=42); print('built X',X.shape,'time',round(time.time()-t0,1),flush=True)
    y=np.array([r['label'] for r in all_records])
    ids=[r['id'] for r in all_records]
    fams=[r.get('family','NEG') for r in all_records]
    pos_idx=np.array([i for i,r in enumerate(all_records) if r['label']==1])
    neg_idx=np.array([i for i,r in enumerate(all_records) if r['label']==0])
    rng=np.random.default_rng(42)
    rows=[]
    for j,fam in enumerate(dist.loc[dist.n_pathways>=10,'family'],1):
        t=time.time()
        test_pos_idx=np.array([i for i in pos_idx if fams[i]==fam])
        train_pos_idx=np.array([i for i in pos_idx if fams[i]!=fam])
        train_neg_idx=rng.choice(neg_idx,size=min(len(neg_idx),2*len(train_pos_idx)),replace=False)
        # ensure test negatives not identical to train negatives where possible
        ...
        train_idx=np.concatenate([train_pos_idx,train_neg_idx]); test_idx=np.concatenate([test_pos_idx,test_neg_idx])
        model=xgb.XGBClassifier(n_estimators=30,max_depth=3,learning_rate=.1,subsample=.8,colsample_bytree=.7,scale_pos_weight=2,min_child_weight=3,reg_alpha=.1,reg_lambda=1.0,eval_metric='logloss',random_state=42+j,n_jobs=4,verbosity=0,tree_method='hist')
        model.fit(X[train_idx],y[train_idx])
        score=model.predict_proba(X[test_idx])[:,1]
        d=metric(y[test_idx],score)
        row={'family':fam,'n_heldout_pathways':len(test_pos_idx),'n_test_negatives':len(test_neg_idx),'n_train_pathways':len(train_pos_idx),'n_go_features':len(selected),'D':len(names),'median_size':float(dist.loc[dist.family==fam,'median_size'].iloc[0]),'median_jaccard':float(dist.loc[dist.family==fam,'median_jaccard'].iloc[0]),**d}
        rows.append(row); print(j,fam,row['test_auroc'],'time',round(time.time()-t,1),flush=True)
